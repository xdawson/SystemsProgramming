...
void _svc_OS_wait(_OS_SVC_StackFrame_t const * const stack) {
	// TODO: need to look into this cause it could be wrong
	while(1) {
		uint32_t tcbState = __LDREXW(&(_currentTCB->state));
		if (_checkCode == stack->r1) {
			uint32_t retCode = __STREXW(tcbState, &(_currentTCB->state));
			if (retCode == 0) {
				break;
			}
		}
